...
import { parseISO, getMonth, getYear, startOfYear, endOfYear, isWithinInterval, format } from 'date-fns';
...
const MonthlyPerformanceCalendar: React.FC<MonthlyPerformanceCalendarProps> = ({ 
  ...
  const monthlyData = useMemo(() => {
    ...
    return monthlyStats;
  }, [trades, currentYear, months, initialCapital]);
  ...
  const equityCurveData = useMemo(() => {
    ...
    const sortedTrades = [...trades]
      .filter(t => {
        ...
        return getYear(tradeDate) === currentYear;
      })
  ...
  }, [trades, currentYear, initialCapital]);
  ...
  return (
    <div className="glass-card p-4 space-y-4 animate-fade-in">
      ...
      <div 
        className="bg-gradient-to-r from-profit/10 to-profit/5 rounded-lg p-4 cursor-pointer border border-profit/20"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-muted-foreground uppercase tracking-wide">
              {currentYear} {language === 'fr' ? 'PERFORMANCE' : 'PERFORMANCE'}
            </p>
            <div className="flex items-center gap-3 mt-1">
              <span className={cn(
                ...
                <ConfidentialValue>
                  {yearlyStats.totalPnl >= 0 ? '+' : ''}{formatAmount(yearlyStats.totalPnl)}
                </ConfidentialValue>
              </span>
              <span className={cn(
                ...
                {yearlyStats.roi >= 0 ? '+' : ''}{yearlyStats.roi.toFixed(1)}% ROI
              </span>
            </div>
          </div>
        ...
        <>
              ...
              {monthlyData.map((month) => (
                <div
                  ...
                  className={cn(
                    ...
                    month.pnl > 0 
                      ? "border-profit/50 bg-profit/5 hover:bg-profit/10" 
                      : month.pnl < 0 
                        ? "border-loss/50 bg-loss/5 hover:bg-loss/10"
          ...
          {viewMode === 'calendar' && (
            <div className="text-center py-8 text-muted-foreground">
              <p className="text-sm">
                {language === 'fr' ? 'Vue calendrier journalier à venir...' : 'Daily calendar view coming soon...'}
              </p>
            </div>
          )}
                  ...
                  .map((month, index) => (
                    <div
                      ...
                      className={cn(
                        ...
                        month.pnl > 0 ? "border-profit/30 bg-profit/5" : "border-loss/30 bg-loss/5"
                      )}
                    >
                      <div className="flex items-center gap-3">
                        <span className={cn(
                        ...
                        <span className="font-medium text-foreground">{month.monthName}</span>
                        <span className="text-xs text-muted-foreground">
                          ({month.tradeCount} {language === 'fr' ? 'trades' : 'trades'})
                        </span>
                      </div>
